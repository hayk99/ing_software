...
public class Tests {
    ...
    private void startTest(){
        try {
            ...
            long id = myNotes.createNote("Some title", "Some body", null);
            logger("TEST 1", id);

            Log.d("TEST 2", "Trying to create note without title ...");
            id = myNotes.createNote("", "Some body",null);
            logger("TEST 2", id);
            ...
            id = myNotes.createNote(null, "Some body",null);
            logger("TEST 4", id);

            Log.d("TEST 5", "Trying to create note body null...");
            id = myNotes.createNote("Some title", null,null);
            logger("TEST 5", id);

            Log.d("TEST 6", "Trying to create correct note and delete the same one...");
            id = myNotes.createNote("Some title", "Some body",null);
    ...
    private void start1002Notes(){
            ...
            for (int i = 0; i < 1002 && success; i++) {
                long id = myNotes.createNote("Note Test 1002 notes " + i, "body of note",null);
                success = id > 0;
            }
    ...
    private void addingAfter1000(){
        ...
        try {
            ...
            long id = myNotes.createNote("Test 15", "creatin new note",null);
            if (id > 0) {
                Log.d("Test 15", "creted");
                try {
                    Log.d("Test 15", "Editing note with rowid: 75");
                    boolean success = myNotes.updateNote(75, "New title for note 75", "the body",null);
                    if (success) {
                        Log.d("Test 15", "first edit completed");
                        try {
                            Log.d("Test 15", "Editing note with rowid: 75");
                            success = myNotes.updateNote(76, "New title for note 76", "the body",null);
    ...
    private void  overflow(){
        ...
        while (works){
            try {
                long id = myNotes.createNote(title, body,null);
                if (id < 1){
